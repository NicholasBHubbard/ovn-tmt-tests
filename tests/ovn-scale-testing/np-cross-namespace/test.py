...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if any(
        ...
        raise ValueError(
            "namespace, pod, timeout, worker and chassis counts must be positive"
        )
    if config["namespaces"] < 2:
        raise ValueError("cross-namespace policy requires at least two namespaces")
    if (
    ...
    if config["total_pods"] > 65534:
        raise ValueError("cross-namespace workload exceeds its endpoint identity space")
...
def test_np_cross_namespace(workload: Any) -> None:
    ...
    for index in range(config["namespaces"]):
        ...
        instance.measure(
            index,
            "startup",
            partial(create_namespace, namespace, index),
        )
    instance.sync()

    def allow_and_verify(index: int) -> None:
        target = (index + 1) % config["namespaces"]
        for family, enabled in ((4, config["ipv4"]), (6, config["ipv6"])):
            if enabled:
                namespaces[index].allow_to(
                    namespaces[target],
                    family,
                    config["allow_priority"],
                )
        instance.verify_connectivity(index * pods, target * pods)

    for index in range(config["namespaces"]):
        instance.measure(
            index,
            "cross_namespace",
            partial(allow_and_verify, index),
        )
    instance.sync()

    print(
        f"Cross-namespace policy passed with {config['namespaces']} namespaces, "
        f"{pods} pods per namespace and {config['total_pods']} policy-managed "
        f"pods across {config['workers']} workers on {config['chassis']} chassis."
    )
